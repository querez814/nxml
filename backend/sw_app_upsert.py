...
def fetch_software_application_stocks():
    try:
        ...
        query = """
            ...
            FROM stocks
            WHERE industry = 'Software - Application';
        """
        cursor.execute(query)
        results = cursor.fetchall()

        stocks = [{"symbol": row[0], "company_name": row[1]} for row in results]
        logging.info(f"Fetched {len(stocks)} stocks for 'Software - Application'.")
        return stocks
...
def upsert_to_pinecone(stocks):
    ...
    vectors = []
    namespace = "industry:software-application"  # Modular namespace for this industry
...
if __name__ == "__main__":
    ...
    software_application_stocks = fetch_software_application_stocks()

    # Step 2: Upsert data to Pinecone
    if software_application_stocks:
        upsert_to_pinecone(software_application_stocks)
    else:

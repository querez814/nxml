...
def fetch_software_application_stocks():
    try:
        ...
        logging.info(f"Fetched {len(stocks)} stocks for 'Semiconductors'.")
        return stocks
...
def upsert_to_pinecone(stocks):
    ...
    vectors = []
    namespace = "industry:semiconductors"  # Modular namespace for this industry
...
if __name__ == "__main__":
    ...
    semi_stocks = fetch_software_application_stocks()

    # Step 2: Upsert data to Pinecone
    if semi_stocks:
        upsert_to_pinecone(semi_stocks)
    else:

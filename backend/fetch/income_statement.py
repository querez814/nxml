...
def get_quarterly_statement_data(ticker: str):
    ...
    numeric_cols = [col for col in result_df.columns if col != "fiscalDateEnding" and col not in margin_cols]
    ...
    for col in numeric_cols:
        ...
        yoy_series = result_df[col].pct_change(periods=4, fill_method=None) * 100
        qoq_series = result_df[col].pct_change(periods=1, fill_method=None) * 100
        yoy_derivative = yoy_series.pct_change(periods=1, fill_method=None) * 100
        qoq_derivative = qoq_series.pct_change(periods=1, fill_method=None) * 100
    ...
    for col in margin_cols:
        ...
        yoy_series = temp_series.pct_change(periods=4, fill_method=None) * 100
        qoq_series = temp_series.pct_change(periods=1, fill_method=None) * 100
        yoy_derivative = yoy_series.pct_change(periods=1, fill_method=None) * 100
        qoq_derivative = qoq_series.pct_change(periods=1, fill_method=None) * 100
...
def get_annual_statement_data(ticker:str):
    ...
    for col in numeric_cols:
        ...
        yoy_series = result_df[col].pct_change(periods=1, fill_method=None) * 100

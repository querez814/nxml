...
<Card class="w-72 border-foreground/25 bg-muted/60 p-4 hover:shadow hover:shadow-foreground/50">
	<div class="flex justify-between">
		<h3 class="font-semibold capitalize tracking-tight">
	...
	<a href={buttonLink} target="_blank">
		<Button size="sm" variant="default" class="my-3 h-7 w-full shadow hover:text-foreground">
			{buttonText}
		</Button>
	</a>
	<div class="my-2 px-1">
	...
	<ul class="space-y-3 px-3 py-3">
		{#each features as feature}
			<li class="flex items-center text-sm">
				{#if feature.included}
					<Check class="mr-2 h-4 w-4 flex-shrink-0 text-primary" />
				{:else}
					<Minus class="mr-2 h-4 w-4 flex-shrink-0 text-muted-foreground" />
				{/if}
